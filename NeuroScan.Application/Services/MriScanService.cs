...
public class MriScanService : IMriScanService
{
    ...
    public async Task<MriScanResponseDTO> UploadAndProcessScanAsync(MriScanUploadDTO uploadDto, Guid userId)
    {
        ...
        var filePath = Path.Combine(_uploadPath, $"{Guid.NewGuid()}_{uploadDto.File.FileName}");
    ...
    public async Task<MriScanResponseDTO> UploadSelfScanAsync(IFormFile file, string? notes, Guid userId)
    {
        ...
        var patient = (await _patientRepository.GetByUserIdAsync(userId)).FirstOrDefault();
        if (patient == null)
        {
            patient = new Patient
            {
                ...
                MedicalRecordNumber = ScanConstants.MrnSelfPrefix + userId.ToString().Substring(0, 8).ToUpper(),
        ...
        var filePath = Path.Combine(_uploadPath, $"{Guid.NewGuid()}_{file.FileName}");
        using (var stream = new FileStream(filePath, FileMode.Create))
        {
    ...
    private async Task EnsureSegmentationSlicesAlignedAsync(MriScan scan, AnalysisResult analysisResult)
    {
        ...
        {
            _logger.LogWarning(exception, "Failed to regenerate segmentation slices for scan {ScanId}", scan.Id);
        }
    }
    ...
    public async Task<IEnumerable<MriScanDetailDTO>> GetScansByPatientIdAsync(Guid patientId, Guid requesterId, bool isDoctor = false)
    {
        ...
        var authorized = isDoctor ? patient.CreatedByUserId == requesterId : patient.UserId == requesterId;
        if (!authorized)
            throw new UnauthorizedAccessException("Patient not found or access denied");
    ...
    public async Task<IEnumerable<MriScanDetailDTO>> GetMyScansAsync(Guid userId)
    {
        ...
        if (patient != null)
        {
            foreach (var scan in await _mriScanRepository.GetByPatientIdAsync(patient.Id))
            {
                var analysisResult = await _analysisResultRepository.GetByMriScanIdAsync(scan.Id);
                scanDetails.Add(new MriScanDetailDTO
                {
    ...
    public async Task SubmitCorrectedMaskAsync(Guid scanId, IFormFile correctedMask, Guid doctorId)
    {
        ...
        var filePath = Path.Combine(_trainingDataPath, $"corrected_{scanId}_{correctedMask.FileName}");
        using (var stream = new FileStream(filePath, FileMode.Create))
        {
    ...
    public async Task<int> GetRawSliceCountAsync(Guid scanId, Guid doctorId)
    {
        ...
        if (!File.Exists(scan.StoredFilePath))
            return 0;
    ...
    public async Task<byte[]?> GetRawSliceAsync(Guid scanId, int sliceIndex, Guid doctorId)
    {
        if (!await _mriImageService.HasRawSliceCacheAsync(scanId, null)
            && await GetRawSliceCountAsync(scanId, doctorId) == 0)
        {
            return null;
        }
    ...
    public async Task SubmitReviewAsync(Guid scanId, Guid doctorId, bool approved, string notes)
    {
        ...
        var analysisResult = await _analysisResultRepository.GetByMriScanIdAsync(scanId);
        if (analysisResult == null)
            throw new InvalidOperationException("No analysis result for this scan");
    ...
    public async Task<PatientEvolutionDTO> GetPatientEvolutionAsync(Guid patientId, Guid requesterId, bool isDoctor = false)
    {
        ...
        var summary = new EvolutionSummaryDTO
        {
            TotalScans = dataPoints.Count
        };
        ...
        if (dataPoints.Count >= 2)
        {
            ...
            summary.BrainVolumeChangeRate = months > 0.0 ? Math.Round(summary.BrainVolumeDelta / months, 4) : 0.0;

            var monthlyLoss = Math.Abs(summary.BrainVolumeChangeRate);
            summary.DegradationLevel = monthlyLoss > 0.05 ? "Severe"
                : monthlyLoss > 0.02 ? "Moderate"
                : monthlyLoss > 0.005 ? "Mild"
                : "Stable";
        }
    ...
    private async Task<string> GenerateEvolutionInterpretation(string patientName, int age, List<EvolutionDataPointDTO> dataPoints, EvolutionSummaryDTO summary)
    {
        var dataPointsSummary = string.Join("\n", dataPoints.Select((dp, i) =>
            $"Scan {i + 1} ({dp.ScanDate:yyyy-MM-dd}): Brain={dp.TotalBrainVolume:F2}cm³, GM={dp.GmVolume:F2}, WM={dp.WmVolume:F2}, CSF={dp.CsfVolume:F2}, Asymmetry={dp.AsymmetryIndex:F2}%, Risk={dp.EpilepsyRiskScore:F1}/100, Tumor={dp.TumorVolume:F3}cm³, Cortex={dp.CortexThicknessAvg:F2}, WM-Density={dp.WmDensityScore:F1}"));
